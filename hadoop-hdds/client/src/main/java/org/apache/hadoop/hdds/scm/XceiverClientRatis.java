...
public final class XceiverClientRatis extends XceiverClientSpi {
  ...
  public XceiverClientReply watchForCommit(long index)
    ...
    try {
      ...
      if (updated < index) {
        LOG.error("reply status {}, index = {}, updated = {}, committedInfos = ", reply.isSuccess(), index, updated);
        commitInfoMap.forEach((k, v) -> LOG.error("{} -> {}", k, v));
        throw new IOException("returned index " + updated + " is smaller than expected " + index);
      }
