...
Expired STS temporary credentials must return ExpiredToken on S3 APIs
    # Requires ozone.test.sts.enabled=true in the STS acceptance compose overlay.
    [Timeout]                     5 minutes
    ...
    Execute S3api Expect Expired Token                          delete-objects --bucket ${ICEBERG_BUCKET_OBS} --delete 'Objects=[{Key=sts-expired-sts-token-delete-objects-marker.txt}],Quiet=false'
